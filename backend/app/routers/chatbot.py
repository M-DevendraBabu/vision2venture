...
def _init_chat():
    ...
    if settings.GROQ_API_KEY:
        try:
            ...
            _chat_provider = "groq"
            print("[Chatbot] OK - Using Groq (qwen/qwen3.8-27b)")
            return
    ...
    if settings.NVIDIA_API_KEY:
        try:
            ...
            print("[Chatbot] OK - Using NVIDIA (meta/llama-3.3-70b-instruct)")
            return
        except Exception as e:
...
async def chat_message(payload: ChatMessage) -> Dict[str, str]:
    ...
    # 1. Try Groq (ultra fast response)
    if settings.GROQ_API_KEY:
        try:
            from groq import Groq
            groq_c = Groq(api_key=settings.GROQ_API_KEY, timeout=8.0)
            ...
            print(f"[Chatbot] Groq call error: {e}. Trying fallback...")
    ...
    if settings.NVIDIA_API_KEY:
        try:
            ...
            completion = nv_c.chat.completions.create(
            ...
            print(f"[Chatbot] NVIDIA fallback error: {e}")
